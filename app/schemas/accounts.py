...
def _validate_password_constratints(password: str):
    ...
    if len(v) <= 6 or len(v) > 256:
        raise ValueError(
            'Password length must be from 6 to 256 characters'
        )

    if v.lower() == v:
        raise ValueError('Password must contain different cases')

    if not [char for char in v if char.isdigit()]:
...
class UserLogIn(BaseUser):
    ...
    password: str = Field(..., description='logging in user password')


class UserRegistration(BaseUser):
    """User pydantic model with data for registration"""

    password1: str = Field(..., description='user password')
    password2: str = Field(..., description='user password repeat')
    ...
    @validator('password2')
    def passwords_match(cls, v, values, **_):
...
class ChangeUserPassword:
    ...
    @validator('new_password')
    def passwords_do_not_match(cls, v, values, **_):
