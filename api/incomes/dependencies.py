...
async def get_all_incomes_for_the_month(
    ...
    db_incomes = await get_all_user_incomes_by_month(user, month)
...
async def create_new_income(
    ...
    async with config.database.transaction():
        creating_income_card = await get_concrete_card(income_data.card_id, user)
        validate_creating_income_amount_currency(income_data, creating_income_card, user)
        created_income = await create_db_income(user, income_data)
    ...
    created_income_scheme = IncomeOut(id=created_income.id, card=creating_income_card,
                                      **income_data.dict())
